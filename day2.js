...
const redAvailable = 12;
const greenAvailable = 13;
const blueAvailable = 14;
const testCaseArray = testCase.split("\n");
let gameNumber = 1;

const testAnswer = testCaseArray.reduce((accumulator, currentValue) => {
  const currentGame = currentValue.split(":").pop().substr(1);
  let redMax = 0, greenMax = 0, blueMax = 0;
  const currentGameResultSet = currentGame.split("; ");
  let gamePossible = true;

  for (let currentGameResult of currentGameResultSet) {
    let currentColorResult = currentGameResult.split(", ");
    for (let currentNumberResult of currentColorResult) {
      let tuple = currentNumberResult.split(" ");
      switch (tuple[1]) {
        case 'red':
          redMax = Math.max(redMax, tuple[0]);
          break;
        case 'green':
          greenMax = Math.max(greenMax, tuple[0]);
          break;
        case 'blue':
          blueMax = Math.max(blueMax, tuple[0]);
          break;
        default:
          break;
      }
  ...
  // Day 2 Part 1
  //if (redMax > redAvailable || greenMax > greenAvailable || blueMax > blueAvailable) {
  //  gamePossible = false;
  //}
  //if (gamePossible) valueToAdd = gameNumber;
  //gameNumber++;

  valueToAdd = redMax * greenMax * blueMax;
