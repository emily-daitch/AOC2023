...
const puzzleInputSmall = 
`32.........441.................367................296........................................567..47.....45.................947...........32
...606..........888.....................508..........*892................+..=138.381..967&..............*....%......926...........218.......
....*......116..*..............747............-....................777..460..........*.......549......127...595.......*..290........*.968...
..902..........425................+..........730..........#...........*.....196.............-.......................512.*....@...994..%.....`;

const puzzleLines = puzzleInput.split('\n');
//const puzzleLines = puzzleInputSmall.split('\n');
console.log('puzzleLine length', puzzleLines[0].length);
let pLNumber = 0;
// add left and right '.' padding
for (puzzleLine of puzzleLines) {
    puzzleLine = '.' + puzzleLine + '.';
    puzzleLines[pLNumber] = puzzleLine;
    pLNumber++;
};
// add top/bottom '.' line padding
//console.log('puzzleLine length', puzzleLines[0].length);
puzzleLines.unshift('.'.repeat(puzzleLines[0].length));
puzzleLines.push('.'.repeat(puzzleLines[0].length));
//console.log('puzzleLines', puzzleLines);

//assuming that puzzleLine length is equal to number of lines
const starFoundCount = new Array(puzzleLines.length);
// for (let i = 0; i < starFoundCount.length; i++) {
//     starFoundCount[i] = new Array(puzzleLines[0].length).fill({
//         count: 0,
//         value: [] 
//     });
// }
...
for (let i = 0; i < starFoundCount.length; i++) {
    starFoundCount[i] = new Array(puzzleLines[0].length);
}
for (let i = 0; i < starFoundCount.length; i++) {
    for (let j = 0; j < starFoundCount[0].length; j++) {
        starFoundCount[i][j] = {
            count: 0,
            value: []
        }
...
for (let currentLineIndex = 1; currentLineIndex < puzzleLines.length -1; currentLineIndex++) {
    //console.log('checking', currentLineIndex);
    let currentCharacterIndex = 1;
    puzzleLine = puzzleLines[currentLineIndex];
    for (; currentCharacterIndex < puzzleLine.length; ) {
        //console.log('currentCharacterIndex', currentCharacterIndex);
        if (!isNaN(puzzleLine[currentCharacterIndex])) {
            const number = parseInt(puzzleLine.substr(currentCharacterIndex));
            const numberLength = String(parseInt(puzzleLine.substr(currentCharacterIndex))).length;
            //console.log('number is ', number);
            //console.log('number length is ', numberLength);

            let numberCounts = false;
            //check left
            if (puzzleLine[currentCharacterIndex - 1] !== '.') {
                //console.log('found left symbol', number);
                if (puzzleLine[currentCharacterIndex - 1] == '*') {
                    console.log('found left *');
                    starFoundCount[currentLineIndex][currentCharacterIndex - 1].count++;
                    starFoundCount[currentLineIndex][currentCharacterIndex - 1].value.push(number);
                }
                numberCounts = true;
            }
            //check right
            if (puzzleLine[currentCharacterIndex + numberLength] !== '.') {
                //console.log('found right symbol', number);
                if (puzzleLine[currentCharacterIndex + numberLength] == '*') {
                    console.log('found right *');
                    starFoundCount[currentLineIndex][currentCharacterIndex + numberLength].count++;
                    starFoundCount[currentLineIndex][currentCharacterIndex + numberLength].value.push(number);
                }
                numberCounts = true;
            }
            for(let currentlyCheckingIndex = currentCharacterIndex; currentlyCheckingIndex < currentCharacterIndex + numberLength; currentlyCheckingIndex++) {
                //check up and down
                //console.log('checking ', currentlyCheckingIndex);
                puzzleLineUp = puzzleLines[currentLineIndex - 1];
                puzzleLineDown = puzzleLines[currentLineIndex + 1];
                if (puzzleLineUp[currentlyCheckingIndex] !== '.' || puzzleLineDown[currentlyCheckingIndex] !== '.') {
                    //console.log('found up or down');
                    if (puzzleLineUp[currentlyCheckingIndex] == '*') {
                        console.log('found up *');
                        starFoundCount[currentLineIndex - 1][currentlyCheckingIndex].count++;
                        starFoundCount[currentLineIndex - 1][currentlyCheckingIndex].value.push(number);
                    }
                    if (puzzleLineDown[currentlyCheckingIndex] == '*') {
                ...
                if (currentlyCheckingIndex == currentCharacterIndex) {
                    if (puzzleLineUp[currentlyCheckingIndex - 1] !== '.' || puzzleLineDown[currentlyCheckingIndex - 1] !== '.') 
                    {
                        //console.log('found up or down left diagonal');
                        //console.log(puzzleLineUp[currentlyCheckingIndex - 1]);
                        //console.log(puzzleLineDown[currentlyCheckingIndex - 1]);
                        if (puzzleLineUp[currentlyCheckingIndex - 1] == '*') {
                            //console.log('found up left *');
                            starFoundCount[currentLineIndex - 1][currentlyCheckingIndex - 1].count++;
                            starFoundCount[currentLineIndex - 1][currentlyCheckingIndex - 1].value.push(number);
                        }
                        if (puzzleLineDown[currentlyCheckingIndex - 1] == '*') {
                            //console.log('found down left *');
                            starFoundCount[currentLineIndex + 1][currentlyCheckingIndex - 1].count++;
                            starFoundCount[currentLineIndex + 1][currentlyCheckingIndex - 1].value.push(number);
                        }
                        numberCounts = true;
                ...
                if (currentlyCheckingIndex == currentCharacterIndex + numberLength - 1) {
                    if (puzzleLineUp[currentlyCheckingIndex + 1] !== '.' || puzzleLineDown[currentlyCheckingIndex + 1] !== '.') 
                    {
                        //console.log('found up or down right diagonal');
                        if (puzzleLineUp[currentlyCheckingIndex + 1] == '*') {
                            //console.log('found up right *');
                            starFoundCount[currentLineIndex - 1][currentlyCheckingIndex + 1].count++;
                            starFoundCount[currentLineIndex - 1][currentlyCheckingIndex + 1].value.push(number);
                        }
                        if (puzzleLineDown[currentlyCheckingIndex + 1] == '*') {
                            //console.log('found down right *');
                            starFoundCount[currentLineIndex + 1][currentlyCheckingIndex + 1].count++;
                            starFoundCount[currentLineIndex + 1][currentlyCheckingIndex + 1].value.push(number);
                        }
                        numberCounts = true;
            ...
            // add to gameTotal if we found a symbol by the number
            // if(numberCounts) gameTotal += number;

            //move past finding
            currentCharacterIndex += String(parseInt(puzzleLine.substr(currentCharacterIndex))).length;
            //console.log('currentCharacterIndex', currentCharacterIndex);
        }
        else {
            //move to next number
...
console.log('starFoundCount', starFoundCount);
for(let j = 0; j < starFoundCount.length ; j++) {
    for(let i = 0; i < starFoundCount[0].length ; i++) {
        //console.log('j, i', j, i);
        if(starFoundCount[j][i].count == 2) {
            //console.log('2 at ', i, j);
            //console.log('values', starFoundCount[j][i].value);
            gameTotal += starFoundCount[j][i].value[0]*starFoundCount[j][i].value[1];
        }
        if(starFoundCount[j][i].count > 2) {
            console.log('>2 at ', i, j, starFoundCount[j][i].count);
        }
    }
}
